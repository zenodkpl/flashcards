...
document.getElementById("htmlChoice").addEventListener("click", () => {
  flashcards = allFlashcards.filter(card => card.topic.startsWith("HTML"));
  flashcards = shuffle(flashcards);
  currentIndex = 0;
  updateFlashcard(currentIndex);

  flashcardContainer.classList.remove('css-flashcards');
  flashcardContainer.classList.add('html-flashcards');

  categoryContainer.style.display = "none";
  flashcardContainer.style.display = "block";
});

document.getElementById("cssChoice").addEventListener("click", () => {
  flashcards = allFlashcards.filter(card => card.topic.startsWith("CSS"));
  flashcards = shuffle(flashcards);
  currentIndex = 0;
  updateFlashcard(currentIndex);

  flashcardContainer.classList.remove('html-flashcards');
  flashcardContainer.classList.add('css-flashcards');
